...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        for i in range(valuelist.count(maxvalue)):
            k = valuelist.index(maxvalue)
            indexlist.append(k)
            valuelist.remove(maxvalue)
        return actions[random.choice(indexlist)]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for f in features:
            qValue += self.weights[f] * featureVectors[f]
        return qValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        featureVectors = self.featExtractor.getFeatures(state, action)
        features = featureVectors.keys()
        for f in features:
            self.weights[f] += self.alpha * diff * featureVectors[f]
